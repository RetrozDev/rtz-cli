...
const removePackages = async () => {
	...
	try {
		const { stdout } = await exec(
			`npm remove ${defaultPackagesToRemove.join(" ")}`,
		);
		console.log(chalk.green(stdout));
	} catch (error) {
		console.warn(
			chalk.yellow(
				"Warning: Some packages could not be removed. They might not be installed.",
			),
		);
	}
};
...
const addScripts = async (root: string) => {
...
export const biomeAction = async (options: BiomeOptions): Promise<void> => {
	try {
